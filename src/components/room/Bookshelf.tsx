...
const SHELF_POS: [number, number, number] = [-11 + 0.2, -5, 5.4]; // back at the wall, base on the floor
const SHELF_W = 2.7;   // width along the wall
const SHELF_H = 3.4;   // total height
const SHELF_D = 0.4;   // depth into the room
...
function VinylCrate(props: React.ComponentProps<"group">) {
  ...
  const setup = useMemo(() => {
    ...
    const e = new THREE.Euler();
    const v = new THREE.Vector3();
    const s = new THREE.Vector3(0.02, 0.62, 0.62);
    const col = new THREE.Color();
    return (mesh: THREE.InstancedMesh | null) => {
      if (!mesh) return;
      sleeves.forEach((sl, i) => {
        e.set(0, 0, sl.rot);
        m.compose(v.set(sl.x, 0.36, 0), q.setFromEuler(e), s);
        mesh.setMatrixAt(i, m);
  ...
  return (
    <group {...props}>
      {/* open crate: bottom + two ends + two rails */}
      <mesh position={[0, 0.03, 0]}>
        <boxGeometry args={[0.72, 0.05, 0.7]} />
        {pine}
      </mesh>
      {[-0.36, 0.36].map((x, i) => (
        <mesh key={i} position={[x, 0.3, 0]} castShadow>
          <boxGeometry args={[0.04, 0.56, 0.7]} />
          {pine}
        </mesh>
      ))}
      {[-0.32, 0.32].map((z, i) => (
        <mesh key={i} position={[0, 0.24, z]}>
          <boxGeometry args={[0.72, 0.16, 0.035]} />
          {pine}
        </mesh>
      ))}
...
function RecordPlayer(props: React.ComponentProps<"group">) {
  ...
  return (
    <group {...props}>
      ...
      {/* two sleeves leaning on the wall behind the player */}
      <mesh position={[0.5, 0.19, -0.21]} rotation={[-0.16, 0, 0.06]} castShadow>
        <boxGeometry args={[0.36, 0.36, 0.012]} />
        <meshStandardMaterial color="#b3111d" roughness={0.6} metalness={0} />
      </mesh>
      <mesh position={[0.53, 0.18, -0.17]} rotation={[-0.2, 0, 0.1]} castShadow>
        <boxGeometry args={[0.36, 0.36, 0.012]} />
...
const Bookshelf = memo(function Bookshelf() {
  ...
  return (
    ...
    <group position={SHELF_POS} rotation={[0, Math.PI / 2, 0]}>
      ...
      <VinylCrate position={[-0.45, 0.08, 0]} />
      <GameStack position={[0.55, 0.085, 0.02]} rotation={[0, -0.06, 0]} />
      {/* trophies: one on the second bay's board end, one on top by the player */}
      <Trophy h={0.52} position={[0.92, 0.08 + 2 * (BAY_H + BOARD_T), 0]} />
      <Trophy h={0.38} position={[-1.02, SHELF_H, 0.02]} />
      {/* scaled 0.8 so the plinth sits on the 0.4-deep top with only a small,
         believable front overhang (full size overhung both edges) */}
      <RecordPlayer position={[0.32, SHELF_H, 0.02]} rotation={[0, -0.04, 0]} scale={0.8} />
      {/* the dino figurines — lengths run ALONG the shelf (their long axis is
         model-space X; keep rotY small or they poke out of the 0.4 depth).
      ...
      <Suspense fallback={null}>
        <DecorModel
          ...
          targetH={0.42}
          position={[-0.5, SHELF_H, 0.02]}
          rotY={0.3}
        />
        <DecorModel
          url="/models/decor/diplodocus.glb"
          targetH={0.28}
          position={[-0.6, 0.08 + 4 * (BAY_H + BOARD_T), 0.04]}
          rotY={-0.2}
        />
      </Suspense>
    </group>
